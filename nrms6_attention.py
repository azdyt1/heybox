...
class AttentionPooling(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        bz = x.shape[0]
        e = self.att_fc1(x)  # (bz, seq_len, 100)
        e = nn.Tanh()(e)
...
class NewsEncoder(nn.Module):
    ...
    def forward(self, inputs):
        title_embed = self.titlelayer(inputs[:, :self.title_size])
        body_embed = self.titlelayer(inputs[:, self.title_size:self.title_size + self.body_size])
...
class MyDataset():
    def __init__(self, args):
        ...
        train_test_data = np.load('train_test_data.npz', allow_pickle=True)
    ...
    def generate_batch_train_data(self):
        ...
        for i in batches:
            ...
            yield (item, user, user_len, self.train_label[i])
